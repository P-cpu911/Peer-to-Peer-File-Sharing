...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		msgBuff[1024] = '\0';
		for(int i = 0; i < MAX_CLIENT_ACTIVE; i++){
			if(read(clientFDs[i], msgBuff, sizeof(msgBuff)) > 0){
				printf("Client %d> %s\nServer>", clientFDs[i], msgBuff);
				//Add write() and message framing (\0) later
			}
			else{
				//Read is woken up through select and fd, but read nothing -> client error/disconnected
